...
- **Порты/маршруты продукта = его манифест** (`omnifield.yaml`, product-owned) — истина там,
  дверь их только регенерит корректно. `registry/ports.md` держит лишь свои хост-порты devopser
  (:8080 дверь / :9443 Portainer), не данные продуктов. Продукт без манифеста просто вне двери (норма).
