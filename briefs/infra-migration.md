...
## ⛔ СНЯТ С БЭКЛОГА (2026-07-09, ревизия user — `devops-consolidated-backlog.md` v2 §2)

«Перенос стеков из оракула» отменён как класс работ: стек появляется у devopser ТОЛЬКО
под точечный заказ продукта-потребителя с его контрактом. Амендмент registry-npm от
2026-07-09 реверснут (registry — пункт скелет-трека, см. бэклог v2 §1.3). Файл оставлен
как история решения; судьба (перефрейм/закрытие) — решение с user.
