...
### ✅ К1 исполнен (owner-workstation, 2026-07-10; ревью architect — апрув)

Коммит `34e3a94`: bootstrap.ps1 — 3 идемпотентных шага (Desktop / дистрибутив с
uid-1000 init / интеграция с рестартом только при изменении), обе грабли вшиты
(BOM-less запись, `wsl --shutdown` против гонки), `-Verify` — 4 read-only чека.
Смоук на настроенной машине: все чеки OK. Полный прогон на чистой тачке — трек
«DoD-прогон workstation» (closeout, не-блокер), там же проверится двухпроходность.
