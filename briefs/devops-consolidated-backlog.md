...
Бэклог принят, приоритеты §1 не оспариваю. Расхождений с существующими брифами не нашёл.
Сделано по факту триажа:

- **Q5 амендмент внесён** в `briefs/infra-migration.md`: + стек `registry-npm`
  (Verdaccio, порт-кандидат 4873) и требование §4 (конвенция под будущие
  сервис-контейнеры — в README стеков при переносе).
- **Сиквенс:** #1 (workstation e2e) ждёт среду от user и Q5 НЕ гейтит — брифы объявлены
  параллельными (`workstation-bootstrap.md` §Порядок). Следующий actionable для architect —
  **#2 Q5**; перед исполнением — blueprint раскладки стеков user'у (канон §Координация).

### §3 Preview-server — вопрос user'у подготовлен (оценка devopser)

**Стоимость взять сейчас:** сам стек тривиален (~полдня: nginx static-serve + volume +
README + ports.md), но `deploy-preview.mjs` регенерится под v2 (+день) и — главное —
**потребителя нет**: продуктов с dist для внешних тестеров в v2 ещё нет, значит DoD
(«стек реально поднимается» + smoke) непроверяем по-честному, стек ляжет мёртвым грузом.
**Стоимость парковки:** нулевая — от preview никто не зависит; в registry уже помечается
«не мигрирован, TBD» (канон Q5-брифа).
**Рекомендация: парковать** до первого продукта с внешними тестерами; тогда это отдельный
мелкий бриф (1–2 дня, стек рядом с gateway). Решение — за user.
