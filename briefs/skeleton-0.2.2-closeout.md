...
- Задачи **1–4** — контур skeleton (шаблон `packages/skeleton/files/`, образ
  `devbox/`, релиз через `.github/workflows/release-devbox.yml`): исполняет
  **owner-skeleton сессия по ЭТОМУ брифу** (запуск — user). Издание 0.2.2 +
  доиздание образа (если Dockerfile-легенда меняет слой) — в её DoD.
- Задача **5** (отмашка brainer) — за architect после издания 0.2.2.
- Поправки П1–П3 и ответы В1/В2 вшиты в `container-sessions-brainer.md`
  (секрет-дизайн один volume+env, Д6 — транзишен-правило, секция вопросов
  заменена резолюцией ревью).
