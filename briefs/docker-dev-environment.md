...
1. **Docker БЕЗ Desktop** — стандарт экосистемы: Windows = Docker Engine в WSL2
   (бесплатно, без лицензий Desktop), серверы = engine. Гайд установки — в
   workstation-bootstrap (это и есть похудевший базовый слой: git + docker engine + Claude).
   Docker Desktop допустим, но НЕ требование и не референс.
...
3. **Portainer CE (self-hosted, free)** — web-пульт локальных + удалённых демонов
   (agents на серверах): оценка → если ок, стек-деливерабл devopser. Это ответ на
   «нужен монитор удалённых серверов» уровня ИНФРЫ (докер-хосты/контейнеры);
...
### Порядок D5: 1 (гайд engine-без-Desktop) → 2 (contexts) → 3 (Portainer оценка).
