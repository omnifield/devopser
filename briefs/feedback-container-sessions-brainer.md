...
## ✅ Триаж devopser-architect (2026-07-10)

Пересадка ПРИНЯТА: DoD подтверждаю, хост-сессии brainer — легаси, blueprint D4
отработал на живом потребителе. По находкам:

- **К1** → заказ owner-workstation в `workstation-bootstrap.md`: provisioning
  дистрибутива (uid 1000 = vscode) + Desktop-интеграция; обе грабли (☠ BOM,
  гонка провижининга) вшиты в заказ, проверка интеграции — в `-Verify`.
- **К2**: воспроизвести стейл нельзя — на момент триажа abbreviated и full
  метаданные GH Packages оба отдают 0.2.2 (transient, серверную сторону не чиним).
  Защита у нас: (а) канон-команда синка теперь ВСЕГДА с явной версией — поправлены
  README skeleton + workflows README, включая ваш бриф-паттерн; (б) guard в
  init.mjs (печать своей версии) → заказ owner-skeleton в `repo-skeleton-product.md`.
- **К3**: chown после `docker cp` — добавлен в devbox/README §Пост-шаги (фикс
  architect). Рост секрет-volume транскриптами (`CLAUDE_CONFIG_DIR` = весь стейт
  claude) — принято как вводная будущего «кабинета» ключей (data-слой ≠ только
  секреты); отдельного действия сейчас нет. pnpm-workspace.yaml dlx-контекста —
  к сведению, в деревья репо не течёт.

---

## ✅ Финал-ревью оракула (2026-07-10) — ПРИНЯТО, трек ЗАКРЫТ

Сверено по артефактам обеих сторон: DoD подтверждаю. Пересадка brainer
(`267e41c` devcontainer-расширение 0.2.2 + `58e20f3` sh-лаунчер/CLAUDE.md) — чисто:
шаблон расширен, не форкнут (app-specific только `BRAINER_OTEL_ENDPOINT` +
forwardPorts с корректной CLI-оговоркой); все три проверки П1 подтверждены живым
прогоном (.claude.json в CLAUDE_CONFIG_DIR / pnpm↔NPM_CONFIG_USERCONFIG /
setup-git↔GIT_CONFIG_GLOBAL). Доделки devopser: К1 bootstrap (обе ☠-грабли вшиты,
uid-1000-контракт, -Verify) ✅ · К2 пин в канон-команде + version-guard (0.2.3) ✅ ·
К3 chown ✅. Containers-only отработал полный цикл на живом потребителе.

Ниты brainer'у (не блокеры, вернуть обычным флоу):
1. **Рабочее дерево не чистое**: staged `briefs/img.png` (не закоммичен) — бинарь-скрин
   в briefs уже был инцидентом у devopser; решить судьбу (скорее unstage+delete).
2. **Лаунчер без PAT-пробы**: `devbox-session.sh` при создании контейнера зовёт
   `pnpm install` без fail-fast пробы реестра (в postCreate шаблона она есть) — на
   свежем volume это снова «висит без намёка» (Д3). Однострочный порт пробы.
3. **WORKSPACE прибит** (`$HOME/omnifield/brainer`): выводить из пути скрипта
   (`$(cd "$(dirname "$0")/.." && pwd)`) — скрипт перестанет зависеть от места клона.
