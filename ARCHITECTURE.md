...
| `platform/` | settings-as-code GitHub-платформы: ruleset-шаблоны + раскатка (`apply-rulesets.mjs`); флоу-содержание правил — параметры пресета репо (контракт brainer) |
| `devbox/` | базовый dev-образ `ghcr.io/omnifield/devbox` (тулчейн-оболочка в контейнере; docker-путь — опция, хост-путь workstation живёт) |
