...
provisioning машин, механизм контрактов (схема манифеста + скан)** — и runtime-инфра строго под заказ потребителя.
...
| `registry/` | СВОИ хост-порты devopser (`ports.md`: :8080/:9443). Контракты продуктов (маршруты/порты) — в их манифестах; devopser держит механизм (схема + скан), не данные продуктов |
...
манифест продукта (агрегируется сканом hub-core), НЕ через хардкод в devopser. Но стек
появляется ТОЛЬКО под точечный заказ продукта-потребителя с его
