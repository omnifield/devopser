...
## Вход в devbox репо (per-repo, канон)

У каждого репо свой devbox (монтирует только себя) — вход двумя командами из корня
репо на хосте (детали и раскладка — [repos.md](repos.md#вход-в-devbox-репо-канон-per-repo)):

```sh
scripts/devbox.sh up               # провижн своего devbox (идемпотентно)
scripts/devbox-session.sh <scope>  # вход агентом (scope: main | <zone>)
```

> ⚠️ Хост-стопгап `~/oa` (docker exec в один devbox с bind'ом ВСЕХ репо) — **retired**:
> god-mount утекал в чужие продукты, нарушал изоляцию single-product-per-devbox. Вход —
> только пара выше; хост-алиас `~/oa` удалить.
