...
> ⚠️ **`.ps1` ниже — мёртвый host-Windows реликт под containers-only. НЕ работает в bash/devbox.**
> Механизм запуска scoped-сессий под контейнер-моделью НЕ определён — эскалирован к хаб-архитектору
> (`briefs/escalation-session-launch-under-containers.md`). Не костылить локально.
