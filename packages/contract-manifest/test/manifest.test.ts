...
test("пример brainer (fullstack, реальные внутренние порты 3500/8010 на /brainer) валиден", () => {
  ...
  // биндинг skeleton-плагина (DEVOPSER-108/166): список ссылок на внешние капабилити.
  assert.deepEqual(parsed.plugins, ["@brainer/agent-harness-plugin@^0.1.0"]);
});

test("plugins: массив строк принимается; отсутствие → undefined (опционально) DEVOPSER-166", () => {
  const withPlugins = ProductManifest.safeParse({
    apiVersion: "omnifield.dev/v1",
    name: "consumer",
    type: "service",
    plugins: ["@brainer/agent-harness-plugin@^0.1.0", "@x/other@^1.0.0"],
  });
  assert.equal(withPlugins.success, true);
  assert.deepEqual(withPlugins.data?.plugins, [
    "@brainer/agent-harness-plugin@^0.1.0",
    "@x/other@^1.0.0",
  ]);
  // опционально: без plugins — валидно, поле undefined.
  const without = ProductManifest.safeParse({
    apiVersion: "omnifield.dev/v1",
    name: "consumer",
    type: "service",
  });
  assert.equal(without.success, true);
  assert.equal(without.data?.plugins, undefined);
});

test("plugins: не-массив / не-строки отклоняются (тонкая ссылка = list of refs) DEVOPSER-166", () => {
  for (const bad of ["@x/one@^1", [{ pkg: "@x/one" }], [123]]) {
    ...
    assert.equal(res.success, false, `plugins=${JSON.stringify(bad)} должно быть отклонено`);
  }
});
