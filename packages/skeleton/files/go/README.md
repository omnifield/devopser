...
Шаг 1 (Foundation): эталон **вживлён в `init.mjs`** — go-набор (`.golangci.yml`,
`sqlc.yaml`) раскатывается на репо go-стека (init-only, продукт правит; drift на них
не гоняется). Стек берётся из `platform/repo-flow.json` (`stack`) с фолбэком на детект
(`go.mod`→go). Свап продуктов на reusable-caller + ретайр их in-repo CI-копий —
handoff chater/brainer (delete-and-call, отдельные PR в их зонах).
