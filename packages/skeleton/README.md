...
| `release` | `active` | универсальная root-agnostic публикация `@omnifield/*` — managed-меха (`release.yml` + `publish-idempotent.mjs`); см. «Release-таргет» |
| `git-flow` | `bound` | вендоренный `git-flow.json` привязан+валидируется; инструмент+rulesets по нему уже есть |

Статус: **`active`** = движок материализует меху (пресетом ИЛИ managed-набором); **`bound`** = пресет
привязан и валидируется, но процессор (материализация/тулинг) ещё не построен; **`declared`** = пустая
plug-in точка. `init.mjs` валидирует declared `target` пресета против таксономии: **unknown target →
loud-fail** (в init и `--check`), и репортит группировку (`[skeleton targets] repo-config: nx, biome,
vite | release: active | git-flow: git-flow` — target без пресет-слотов, но с managed-мехой виден
статусом из таксономии, а не «—»).
...
## Release-таргет — универсальная root-agnostic публикация (DEVOPSER-194, knowledger `ADR-17`)

Публикация npm-пакетов `@omnifield/*` — **универсальная workspace-меха, root-agnostic**: ОДНА форма
на любой рут-стек (go / node / python / полиглот), ноль ветвления под продукт или стек. Две managed
записи (`mode:exact`, drift-managed, каскадят ВСЕМ репо **без стек-гейта**):

| dest | что |
|---|---|
| `.github/workflows/release.yml` | `workflow_dispatch`-джоба publish (`GITHUB_TOKEN packages:write`). **Root-agnostic:** node/pnpm — рантайм публикации (версии пиннятся **в workflow**, НЕ `node-version-file`/`packageManager` из корня); корневой `package.json`/`pnpm-lock` **не требуется**. |
| `scripts/publish-idempotent.mjs` (+ `.test.mjs`) | скан **ПО ПАКЕТАМ, не по корню**: находит все `@omnifield/*` не-`private` пакеты ГДЕ БЫ ни лежали (пакет в подпапке go/python-корня — ок, БЕЗ корневого pnpm-workspace), издаёт каждый идемпотентно (`pnpm publish` в его директории). |

**Идемпотентность (DEVOPSER-132).** Новые версии выкатываются; уже опубликованная версия → **skip с
логом** (не fail — реестр источник правды, conflict публикации трактуется как дубликат). Партиал-бамп
(1 новый пакет, остальные дубликаты) → джоба **зелёная**. Ноль публикуемых `@omnifield/*` → **зелёный
no-op** (не падает). Genuine publish-fail (auth/сеть) → джоба красная.

**Провайдер самообъявляется** наличием пакета `@omnifield/<product>-<capability>` — devopser не ведёт
списка провайдеров и не ветвит go/node (движок держит МЕХАНИЗМ, не данные). Запуск:
...
Оба — в одном PR (иначе биндинг разъедется с публикуемой версией). Публикация — **release-таргетом**
(managed `release.yml` + `publish-idempotent.mjs`, GitHub Packages `npm.pkg.github.com`; см.
«Release-таргет»); версия = `package.json` пресета, бамп **ручной**, zero-dep (без changeset/nx
release). Потребители подтягиваются сами: их `--check` краснеет на новом ранге → `init` синкает.
...
| `.github/workflows/release.yml` · `scripts/publish-idempotent.mjs` · `scripts/publish-idempotent.test.mjs` | release-таргет — универсальная root-agnostic публикация `@omnifield/*` (без стек-гейта, любой стек; DEVOPSER-194 / ADR-17) — точная копия эталона |
