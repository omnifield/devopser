...
- **Маршруты** (`nginx.conf`): `/brainer/` — фронт (vite за префиксом, HMR
  websocket проброшен); `/api/brainer/` — бэк (SSE без буферизации). До
  исполнения порт-контракта owner'ами brainer (3500/8010, их брифы
  `gateway-parity-*`) маршруты отвечают 502 — ожидаемо, gateway поднимается
  первым.
