...
### Standalone-фронт (vite/pnpm-воркспейс, БЕЗ nx)

`ci.yml` (фронт в сабдире `web/`; для фронта в корне — убрать `with`):
...
concurrency:
  group: ${{ github.workflow }}-${{ github.ref }}
  cancel-in-progress: true
# web-флоу не тянет nx-set-shas/@omnifield → достаточно contents: read (как go).
permissions: { contents: read }
jobs:
  web:
...
Обязательные файлы у web-потребителя (standalone-фронт): в каталоге `working-directory`
(дефолт корень) — `package.json` с пинами `packageManager` (pnpm) + `engines.node`,
`pnpm-lock.yaml` (install `--frozen-lockfile`), pnpm-скрипты `lint` / `typecheck` / `test` /
`build`. Ноль nx. `web-ci` берёт pnpm/node из пинов ИМЕННО этого воркспейса
(`package_json_file` / `node-version-file` = `<wd>/package.json`).
...
## Грабли Web (`web-ci.yml`, standalone-фронт)

- **`working-directory`-инпут** (дефолт `.`): фронт в сабдире (`web/`) передаёт его —
  применяется к `install`/`lint`/`typecheck`/`test`/`build` и путям `package_json_file` /
  `node-version-file` / `cache-dependency-path`. Секрет-скан (gitleaks) — всегда от корня репо.
- **Пины — из воркспейса, не из корня**: `pnpm/action-setup` читает `packageManager` через
  `package_json_file: <wd>/package.json`; `setup-node` — `engines.node` оттуда же. Пин в корневом
  `package.json` (если он есть) НЕ участвует — источник только воркспейс фронта.
- **pnpm-скрипты обязательны**: `web-ci` зовёт `pnpm run {lint,typecheck,test,build}` — воркспейс
  обязан их определить (`test` — CI-форма, напр. `vitest run`, не watch). Отсутствие скрипта = красный.
- **Ноль nx**: web-ci — не для nx-монорепо. nx-репо в корне держит `node`-стек → `node-ci`;
  standalone-фронт → `frontend`-стек → `web-ci` (разведение в `init.mjs` по `repo-flow.json`).
- **permissions caller = `contents: read`**: как go — web-флоу не тянет nx-set-shas/@omnifield.
